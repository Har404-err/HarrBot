...
// Taruh kode ini di dalam file /lib/game.js

const Bansos = async (m, db) => {
    let user = db.users[m.sender];
    const cooldown = 7200000; // 2 jam dalam milidetik

    // Cek apakah properti lastbansos sudah ada, jika tidak, inisialisasi
    if (user.lastbansos === undefined) {
        user.lastbansos = 0;
    }

    let lastbansos = user.lastbansos;
    let sekarang = Date.now();

    if (sekarang - lastbansos < cooldown) {
        let sisa = (lastbansos + cooldown) - sekarang;
        return m.reply(`Anda baru saja mencoba korupsi!\nSilakan tunggu *${clockString(sisa)}* lagi.`);
    }

    // Set cooldown di awal untuk mencegah multiple request
    user.lastbansos = sekarang;

    // Peluang 50% untuk berhasil
    let sukses = Math.random() < 0.5;

    if (sukses) {
        // Jika berhasil, dapat uang antara 5 juta sampai 20 juta
        let hadiah = Math.floor(Math.random() * 15000000) + 5000000;
        addMoney(hadiah, m.sender, db);
       const imagePathSukses = path.join('./src/media/berhasil.jpg');
        const captionSukses = `🎉 *BERHASIL!* 🎉\n\nAnda berhasil melakukan korupsi dana bansos dan mendapatkan uang sebesar *Rp${hadiah.toLocaleString('id-ID')}*!`;
        
        // Membaca file gambar ke dalam buffer
        const imageBufferSukses = fs.readFileSync(imagePathSukses);
        
        // Mengirim buffer gambar beserta caption
        await m.reply({ image: imageBufferSukses, caption: captionSukses });
        
    } else {
        if (user.money < 100000) {
             return m.reply(`🕵️‍♂️ *TERCIDUK KPK!* 🕵️‍♂️\n\nAnda gagal korupsi, tetapi karena uang Anda sedikit, Anda hanya diberi peringatan.`);
        }
        let denda = Math.floor(user.money * 0.75);
        setMoney(user.money - denda, m.sender, db);

        // --- KODE DIUBAH UNTUK MENGAMBIL FILE LOKAL ---
        const imagePathGagal = path.join('./src/media/gagal.jpg');
        const captionGagal = `🕵️‍♂️ *TERCIDUK KPK!* 🕵️‍♂️\n\nAnda gagal dan tertangkap tangan! Anda didenda sebesar 75% dari kekayaan Anda, yaitu *Rp${denda.toLocaleString('id-ID')}*.`;

        // Membaca file gambar ke dalam buffer
        const imageBufferGagal = fs.readFileSync(imagePathGagal);
        
        // Mengirim buffer gambar beserta caption
        await m.reply({ image: imageBufferGagal, caption: captionGagal });
    }
}
...
module.exports = { rdGame, iGame, tGame, gameSlot, gameCasinoSolo, gameSamgongSolo, gameMerampok, gameBegal, daily, buy, setLimit, addLimit, addMoney, setMoney, transfer, Blackjack, SnakeLadder, Bansos }
