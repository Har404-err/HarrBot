...
const korupsi = async (naze, m, db) => {
    ...
    if (sukses) {
        ...
        addMoney(hadiah, m.sender, db);
                  const imagePath = path.join('./src/media/berhasilkorupsi.jpg');
        const caption = `🎉 *BERHASIL!* 🎉\n\nAnda berhasil melakukan korupsi dana  dan mendapatkan uang sebesar *Rp${hadiah.toLocaleString('id-ID')}*!`;
        try {
        ...
        // --- KODE DIUBAH UNTUK MENGAMBIL FILE LOKAL ---
                const imagePathGagal = path.join('./src/media/gagalkorupsi.jpg');
...
module.exports = { rdGame, iGame, tGame, gameSlot, gameCasinoSolo, gameSamgongSolo, gameMerampok, gameBegal, daily, buy, setLimit, addLimit, addMoney, setMoney, transfer, Blackjack, SnakeLadder, korupsi }
