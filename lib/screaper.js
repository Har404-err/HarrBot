...
async function animeSearch(query) {
    return new Promise(async (resolve, reject) => {
        try {
            const { results } = await google.search(`site:myanimelist.net ${query}`, {
                safe: false,
                parse_ads: false,
                additional_params: {
                    hl: 'en'
            ...
            if (!results.length) return reject('No results found.');
            const { title, url, description } = results[0];
            ...
            const genres = [];
            $('span[itemprop="genre"]').each((i, el) => {
                genres.push($(el).text());
            });
            resolve({ title, url, thumbnail, synopsis, rating, genres });
        } catch (e) {
            reject(e);
        }
    });
...
module.exports = { pinterest, wallpaper, remini, hitamkan, wikimedia, yanzGpt, mediafireDl, ringtone, styletext, instagramDl, tiktokDl, facebookDl, instaStalk, telegramStalk, tiktokStalk, genshinStalk, instaStory, bk9Ai, spotifyDl, ytMp4, ytMp3, NvlGroup, quotedLyo, youSearch, gptLogic, savetube, simi, geminiAi, animeSearch }
